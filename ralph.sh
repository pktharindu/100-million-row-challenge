...
$(cat "$SCRIPT_DIR/.agent/PROMPT.md")"
  ...
  script -q "$OUTPUT_FILE" bash -c 'claude --dangerously-skip-permissions --model opus --output-format stream-json --verbose -p "$PROMPT_CONTENT"' >/dev/null 2>&1 &
  AGENT_PID=$!
  ...
  # Auth error detection: Instead of grepping output content (which false-positives on
  # projects that discuss auth features), check if the claude process produced almost
  # no output — a real auth failure exits immediately with a tiny error, while a
  # working iteration produces thousands of lines of JSON stream data.
  OUTPUT_SIZE=$(wc -c < "$OUTPUT_FILE" 2>/dev/null | tr -d ' ')
  if [ "${OUTPUT_SIZE:-0}" -lt 500 ] && [ -z "$(grep -l '"type"' "$OUTPUT_FILE" 2>/dev/null)" ]; then
    # Tiny output with no JSON stream = CLI failed to start (auth, network, binary missing, etc.)
    stop_spinner
    clear_rolling_preview
    CLI_ERROR=$(cat "$OUTPUT_FILE" 2>/dev/null | tr -d '\000-\010\013\014\016-\037' | head -5)
    echo ""
    echo -e "${RD}░░▒▒▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▒▒░░${R}"
    echo -e "  ❌ ${RD}Claude CLI Error${R}"
    if [ -n "$CLI_ERROR" ]; then
      echo -e "  $CLI_ERROR"
    else
      echo -e "  Claude produced no output. Check ${C}claude login${R} or network."
    fi
    echo -e "${RD}░░▒▒▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▒▒░░${R}"
  ...
  if [ -n "$FINAL_SUMMARY" ]; then
    display_final_summary "$FINAL_SUMMARY" 20
  else
    # Fallback: show last 20 lines of parsed output if no result found
    FALLBACK_SUMMARY=$(echo "$OUTPUT" | tail -n 20)
    if [ -n "$FALLBACK_SUMMARY" ]; then
      display_final_summary "$FALLBACK_SUMMARY" 20
    fi
  fi
